...
sub send_200 {
    ...
    my $html            = shift;
    $self->{http_request}->respond ([200, 'OK', { 'Content-Type' => $content_type , $self->$_get_set_cookie() }, $html]);
}
...
sub send_404 {
    ...
    my $content_type    = shift;
    my $html            = shift;
    $self->{http_request}->respond ([404, 'Not Found', { 'Content-Type' => $content_type , $self->$_get_set_cookie() }, $html]);
}
